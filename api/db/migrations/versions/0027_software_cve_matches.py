...
**Chain note.** This revision is numbered 0027 because 0026 is being added in
parallel on another branch. That revision does not exist in this tree, so
``down_revision`` points at 0025 — the head this branch was cut from — rather
than at a revision alembic could not resolve, which would fail every migration
run and every test that builds a schema. On merge, re-point ``down_revision``
at 0026's actual revision id (this repository's ids are ``NNNN_slug``, not bare
``NNNN``) so the chain stays linear; nothing else about this revision changes.
