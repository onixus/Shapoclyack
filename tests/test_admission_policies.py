...
def test_the_release_pipeline_signs_and_checks_with_the_committed_public_key():
    # The pre-flight and the signing step both name cosign.pub; the policies'
    # secret is documented as created from that same file.
    assert JENKINS_PUBLISH.count("--pubkey cosign.pub") == 2
...
def test_both_policies_require_slsa_v1_provenance_from_the_same_identities():
    ...
    for authority in CIP["spec"]["authorities"]:
        assert [a["predicateType"] for a in authority["attestations"]] == ["slsaprovenance1"]
    # …which is what the script attests, from what both builds record.
    assert SIGN_SCRIPT.count("--type slsaprovenance1") == 2
...
def test_the_release_pipeline_pushes_by_digest_and_cannot_skip_signing():
    ...
    assert "scripts/sign-release-image.sh" in sign.split("if (params.DRY_RUN)")[0]
