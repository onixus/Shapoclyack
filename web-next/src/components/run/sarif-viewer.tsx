...
export function SarifViewerDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl h-[85vh] flex flex-col p-6 bg-muted border-border text-foreground shadow-2xl">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <DialogTitle className="text-base font-bold text-foreground font-mono">
                  SARIF 2.1.0 Security Report · <span className="text-sky-600 dark:text-sky-400">{runId}</span>
                </DialogTitle>
        ...
        <div className="grid grid-cols-4 gap-3 py-3 border-b border-border">
          ...
          <div className="flex items-center gap-2.5 rounded-lg bg-sky-950/20 p-2.5 border border-sky-900/30">
            ...
            <div>
              ...
              <p className="text-sm font-bold font-mono text-sky-600 dark:text-sky-300">
        ...
        >
          <div className="flex flex-wrap items-center justify-between gap-3 pb-2">
            <TabsList className="bg-card border border-border p-1">
              <TabsTrigger value="findings" className="text-xs font-semibold data-[state=active]:bg-sky-500/20 data-[state=active]:text-sky-600 dark:text-sky-300 gap-1.5">
                <ShieldAlert className="h-3.5 w-3.5" />
                Findings ({counts.total})
              </TabsTrigger>
              <TabsTrigger value="rules" className="text-xs font-semibold data-[state=active]:bg-sky-500/20 data-[state=active]:text-sky-600 dark:text-sky-300 gap-1.5">
                <BookOpen className="h-3.5 w-3.5" />
                Rules Catalog ({counts.rules})
              </TabsTrigger>
              <TabsTrigger value="raw" className="text-xs font-semibold data-[state=active]:bg-sky-500/20 data-[state=active]:text-sky-600 dark:text-sky-300 gap-1.5">
              ...
              <div className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="h-8 w-60 pl-8 text-xs bg-card border-border text-foreground"
                  />
                </div>
            ...
            {filteredResults.length === 0 ? (
              ...
              filteredResults.map((res, idx) => {
                ...
                return (
                  <div
                    key={`${res.ruleId}-${idx}`}
                    className="p-3.5 rounded-xl border border-border bg-card hover:bg-card transition-colors space-y-2"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <Badge
                          ...
                          className={
                            lvl === "error"
                              ? "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/30 font-bold font-mono text-[10px]"
                              : lvl === "warning"
                              ? "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/30 font-bold font-mono text-[10px]"
                              : "bg-sky-500/10 text-sky-600 dark:text-sky-400 border-sky-500/30 font-bold font-mono text-[10px]"
                          }
                        >
                          {lvl.toUpperCase()}
                        </Badge>
                        ...
                        {rule?.name && rule.name !== res.ruleId && (
                          <span className="text-xs text-muted-foreground font-medium">· {rule.name}</span>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 flex-wrap">
                        {cve && (
                          <Badge variant="secondary" className="bg-muted text-rose-600 dark:text-rose-300 font-mono text-[10px]">
                            {cve}
                          </Badge>
                        )}
                        {cweList.slice(0, 2).map((cwe) => (
                          <Badge key={cwe} variant="secondary" className="bg-muted text-sky-600 dark:text-sky-300 font-mono text-[10px]">
                            {cwe}
                          </Badge>
                        ))}
                      </div>
                    ...
                    {locationUri && (
                      <div className="flex items-center gap-1.5 font-mono text-[11px] text-muted-foreground bg-muted px-2.5 py-1 rounded-lg border border-border">
                        <span className="text-muted-foreground font-semibold">Target URI:</span>
                        <span className="text-sky-600 dark:text-sky-300 truncate">{locationUri}</span>
                      </div>
                    )}
              ...
              rules.map((rule) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="font-mono text-[10px] text-sky-600 dark:text-sky-400 border-sky-500/30">
                        RULE
                      </Badge>
                  ...
                  {rule.shortDescription?.text && (
                    <p className="text-xs text-foreground">{rule.shortDescription.text}</p>
                  )}
                  {rule.fullDescription?.text && rule.fullDescription.text !== rule.shortDescription?.text && (
                    <p className="text-xs text-muted-foreground">{rule.fullDescription.text}</p>
                  )}
                  {rule.help?.text && (
                    <div className="text-xs text-muted-foreground bg-muted p-2.5 rounded-lg border border-border">
          ...
          <TabsContent value="raw" className="flex-1 overflow-hidden flex flex-col">
            <pre className="flex-1 overflow-auto p-4 rounded-xl bg-muted border border-border font-mono text-[11px] text-foreground leading-relaxed select-text">
              {sarifText}
            </pre>
          </TabsContent>
        </Tabs>
