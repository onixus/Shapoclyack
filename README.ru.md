...
* **Remote Agents (Удаленные агенты):** автономные воркеры в демилитаризованных зонах (DMZ) и изолированных облачных сегментах (VPC). Агенты инициируют только исходящее соединение — к брокеру NATS либо по HTTPS к API — и не требуют открытия входящих портов в защищенный сегмент. Соединение с NATS пока идет без TLS ([#309](https://github.com/onixus/Shapoclyack/issues/309), [#359](https://github.com/onixus/Shapoclyack/issues/359)), поэтому брокер держат внутри доверенного сегмента; взаимная аутентификация по сертификатам (mTLS) — в roadmap.
...
| **Kubernetes (`overlays/prod`)** | Промышленная эксплуатация, мультиагентность | Отдельные Deployment'ы: API (сейчас одна реплика), PostgreSQL, пул агентов. NATS и ClickHouse в этом оверлее выключены пустыми URL, HA-профиль — [#335](https://github.com/onixus/Shapoclyack/issues/335) |
