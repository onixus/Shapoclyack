...
`PUT /api/tenants/{tenant_id}/members/{username}`. В консоли пока доступны
только `viewer`, `operator` и `admin`: редактор членств держит свой список
ролей, и новые пять выдаются через API.
