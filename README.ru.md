...
Стадийный распределенный сетевой сканер, FastAPI control plane, защищённые сенсоры (удалённые узлы сканирования), долговечный инвентарь активов, обогащение контекстом угроз (EPSS, CISA KEV) и бюллетенями вендоров, аналитика на ClickHouse и операторская консоль на Next.js 14 — разворачивается как единый All-in-One контейнер или масштабируемый отказоустойчивый кластер Kubernetes.
...
> [!NOTE]
> **Терминология.** **Сенсор** — удалённый узел сканирования: запускает `agent/worker.py`, забирает задания у API (по NATS или HTTPS), выполняет сканер и загружает результаты; в реестре `agents` он записан с `agent_kind = "scanner"`. **Агент** — endpoint-агент Lariska, установленный на управляемом хосте: отправляет снимки инвентаря в `POST /api/endpoint/inventory` и никогда не берёт задания сканирования; в том же реестре `agents` он записан с `agent_kind = "endpoint"`. **Конечная точка (эндпоинт)** — управляемый хост с установленным агентом. Слово «агент» без уточнения в документации всегда означает агента Lariska; всё, что сканирует, — сенсор. Идентификаторы в коде (пакет `agent/`, таблица `agents`, маршруты `/api/agents/*`, переменные `OCTO_AGENT_*`, страница консоли `/agents`) не переименовывались.
...
* Для уязвимостей ПО хостов закрытие подтверждается следующим снимком инвентаризации от агента (Lariska).
...
### 4. Анализ ПО конечных точек (агент Lariska) с учетом бэкпортов вендоров
При анализе установленного на серверах ПО Shapoclyack сопоставляет пакеты не с абстрактными диапазонами NVD CPE (дающими массовые ложные срабатывания из-за бэкпортов патчей безопасности в стабильные дистрибутивы), а с официальными бюллетенями безопасности вендоров (Ubuntu USN, Debian Security Tracker). Хосты Windows оцениваются по другой оси — по **сборке ОС** (`10.0.<build>.<ubr>`) против ремедиаций Microsoft Security Update Guide, потому что бюллетень Microsoft пишется про сборку, а не про версию пакета; сторонние продукты Windows при этом не сопоставляются и честно помечаются как инвентарь. Платформа формирует **Patch Gaps** — конкретные пакеты и готовые консольные команды обновления (`apt-get install --only-upgrade <pkg>=<fixed_version>`), минимизируя трудозатраты системных инженеров. Подробнее: [docs/software-cve-matching.md](docs/software-cve-matching.md).

### 5. Автоматизированный аудит соответствия стандартам (Compliance)
Непрерывная автоматическая оценка статуса контролей PCI DSS 4.0, CIS Controls v8, ISO/IEC 27001:2022, приказов ФСТЭК России № 117 (ГИС), № 21 (ИСПДн) и № 239 (КИИ) и ГОСТ Р 57580.1-2017 на базе фактических свидетельств (evidence) сканирования и инвентаря конкретного тенанта. Российские каталоги ведутся по кодам мер регуляторов (АНЗ.1, АУД.2, ЦЗИ.8 …) и по срокам устранения из Руководства ФСТЭК (24 ч / 7 дн / 4 нед / 4 мес), а не по SLA тенанта. Готовые объективные отчеты для внутренних аудиторов, топ-менеджмента и регуляторов. Подробнее: [docs/reports-and-compliance.md](docs/reports-and-compliance.md).
...
| 🏛️ [**Сценарии для Архитектора ИБ**](docs/wiki/scenarios-architect.md) | Системный уровень: инвентаризация периметра (EASM), выявление Shadow IT, REST-контракт бизнес-контекста активов для скрипта синхронизации с CMDB/AD (готового импортера пока нет — [#350](https://github.com/onixus/Shapoclyack/issues/350)), CI/CD DevSecOps, SIEM/SOAR, распределённая топология сенсоров (DMZ/VPC), контроль сегментации, комплаенс. |
...
| **Инвентарь (CAASM)** | Долговечные активы между запусками, FQDN/hash/cert-идентификаторы, владение, критичность, жизненный цикл, инвентаризация ПО конечных точек агентом Lariska |
| **Управление уязвимостями (RBVM)** | Учет находок со статусами (`OPEN`, `ACKNOWLEDGED`, `PLANNED`, `FIXING`, `VERIFYING`, `CLOSED`), SLA по критичности, исключения; двухосевой риск NIST SP 800-30; канбан-доска ремедиации с инструментальной верификацией и синхронизацией тикетов (push автоматический, обратная проверка закрытия — по запросу до [#347](https://github.com/onixus/Shapoclyack/issues/347)) |
...
| **Аудит комплаенса** | Автоматическая оценка статуса контролей PCI DSS 4.0, CIS Controls v8, ISO/IEC 27001:2022, приказов ФСТЭК № 117 / 21 / 239 и ГОСТ Р 57580.1-2017 на основе фактических свидетельств тенанта |
...
| **Эксплуатация** | Управление заданиями (Jobs) и расписаниями (Cron), сравнение прогонов (diff), алерты в мессенджеры/почту, распределённые сенсоры, продолжение прерванных сканов (resume) |
...
graph TB
    ...
    subgraph DataPlane ["Data Plane (сканеры, сенсоры и агенты)"]
        LocalScanner["Локальный сканнер (Job/Pod)"]
        Sensor1["Сенсор (контур DMZ)"]
        Sensor2["Сенсор (изолированный VPC)"]
        Lariska["Агент Lariska (ПО конечных точек)"]
    end
    ...
    NATS <-->|Token (Outbound)| Sensor1
    NATS <-->|Token (Outbound)| Sensor2
...
* **NATS JetStream:** распределенная отказоустойчивая шина обмена сообщениями. Задания сканирования буферизуются и доставляются сенсорам с гарантией доставки.
* **Next.js 14 Console:** веб-интерфейс оператора со статическим экспортом, не требующий Node.js сервера в production.
* **Сенсоры (ресурс API `agents`, `agent_kind = scanner`):** автономные узлы сканирования в демилитаризованных зонах (DMZ) и изолированных облачных сегментах (VPC). Сенсоры инициируют только исходящее соединение — к брокеру NATS либо по HTTPS к API — и не требуют открытия входящих портов в защищенный сегмент. TLS до NATS включается явно (`tls://` и `OCTO_NATS_TLS_*`, см. [configuration.md](docs/configuration.md)) — включайте его до выхода за пределы доверенного сегмента; взаимная аутентификация сенсоров по сертификатам (mTLS) не реализована.
...
* **Инвентарь конечных точек и Patch Gaps:** реестр конечных точек с установленным агентом Lariska, сопоставление установленных пакетов с официальными бюллетенями безопасности и готовые команды обновления.
...
* **Фабрика отчетов и Комплаенс:** генерация брендированных отчетов для руководства и технических специалистов, матрица соответствия контролям PCI DSS 4.0, CIS Controls v8, ISO 27001, приказов ФСТЭК № 117 / 21 / 239 и ГОСТ Р 57580.1.
...
* **Администрирование:** управление тенантами, квотами, парком сенсоров (страница «Агенты», маршрут `/agents`), интеграциями с таск-трекерами, словарями (wordlists) и безопасными параметрами конфигурации.
...
| **Kubernetes (`overlays/prod`)** | Промышленная эксплуатация | Отдельные Deployment'ы: API (одна реплика), PostgreSQL; пул сенсоров — отдельный оверлей `overlays/agents`. NATS и ClickHouse в этом оверлее выключены пустыми URL |
...
| `risk-approver` | Утверждает и отклоняет заявки на принятие риска, а также снимает уже действующее принятие ([#348](https://github.com/onixus/Shapoclyack/issues/348)) |
...
`PUT /api/tenants/{tenant_id}/members/{username}` или в консоли, на вкладке
«Tenant membership»: список ролей там приходит из `GET /api/rbac/roles`, то
есть из каталога самой платформы, вместе с описанием каждой роли.
...
Документация соответствует релизу [`shapoclyack-0.45-0916`](https://github.com/onixus/Shapoclyack/releases/tag/shapoclyack-0.45-0916).
...
| `ghcr.io/onixus/shapoclyack-scanner` | Data Plane: сканер и среда выполнения сенсора (`python -m agent`) |

> [!TIP]
> В производственной среде обязательно фиксируйте конкретный тег релиза (например, `0.45-0916`) и не используйте плавающий тег `latest`.
