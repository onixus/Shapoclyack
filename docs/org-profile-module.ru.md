...
- promote держится за утверждённый scan scope (#226) дважды: в момент
  продвижения (`403`, запись в `auth_events` — атрибуция решается оператором,
  авторизация админом) и в момент старта скана (домен, который scope с тех пор
  перестал покрывать, выбрасывается и фиксируется на job'е как
  ...
  что и набранная цель;
- продвижение отзываемо: `DELETE /runs/{id}/related-domains/{domain}/promote`
  (operator+) — таблица рисков ниже говорит, что ошибка атрибуции есть скан
  чужой инфраструктуры, поэтому promote без undo неприемлем;
...
- `POST /runs/{id}/related-domains/{domain}/promote` → добавить в цели всех
  ...
  утверждённый scan scope домен не покрывает); `DELETE` по тому же пути —
  отозвать; `promoted_domains` в ответе `org-profile` — кандидаты этого run'а,
  которые тенант продвинул (читается из таблицы, не из run'а);
- `GET /tenants/{id}/promoted-domains` (admin) — что операторы добавили под
  утверждённый scope: домен, run-источник, кто и когда;
...
- вкладка «Org Profile» там же (`components/run/related-domains.tsx`) —
  владелец, связанные домены с evidence, кнопка «Promote to Scope», которая у
  продвинутого домена становится «Withdraw from Scope»; отказ scope (`403`)
  показывается баннером, а не глотается. Отдельной страницы `/org-profile`
  нет — Track C закрыт, новых IA не заводим (ROADMAP, *Not doing*);
