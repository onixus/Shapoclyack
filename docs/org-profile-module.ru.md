...
- продвижение домена в scope вручную — отдельное действие оператора в UI/API
  ...
  кандидата (`related_domains.json`), иначе `400`. Решение хранится **на
  тенанте** (`tenant_promoted_domains`, миграция `0031`), а не в run'е: run
  удаляет retention (#187), решение оператора его переживает. Каждый обычный
  скан тенанта (`jobs.start_scan`, в том числе по расписанию) получает эти
  домены отдельным входом `--promoted-domains`, который сканер **добавляет** к
  своему FQDN-scope — run на дефолтных файлах целей расширяется, а не
  перенацеливается. Verification-перескан (#183) не расширяется никогда: он
  нацелен на одну находку;
- promote держится за утверждённый scan scope (#226) дважды и обеими половинами
  проверки — суффиксной и resolve-time (`OCTO_SCAN_SCOPE_RESOLVE_CHECK`), как
  набранная цель: в момент продвижения (`403`, запись в `auth_events` —
  атрибуция решается оператором, авторизация админом) и в момент старта скана
  (домен, который scope с тех пор перестал покрывать или чей адрес попал в
  deny-диапазон, выбрасывается и фиксируется на job'е как
  `scan_options.promoted_domains_refused`, а не срывает скан по собственным
  целям оператора); внутри сканера он проходит тот же фильтр `--scan-scope`,
  что и набранная цель. Проверка — одна реализация (`scan_scope.filter_names`),
  а не копия;
- продвижение отзываемо **без run'а**: `DELETE /promoted-domains/{domain}`
  (operator+, ключ — тенант и домен), плюс тот же undo с вкладки run'а —
  `DELETE /runs/{id}/related-domains/{domain}/promote`, где run только называет
  тенанта. Run, предложивший домен, удаляет retention (#187), а продвинутый
  домен на следующем run'е становится seed и кандидатом больше не бывает —
  поэтому undo, привязанный к run'у или к списку кандидатов, через месяц
  перестал бы существовать. Таблица рисков ниже говорит, что ошибка
  атрибуции есть скан чужой инфраструктуры, поэтому promote без undo
  неприемлем;
- оба направления журналируются в `auth_events` с актором
  (`trust_change` / `promoted_domain_added`, `promoted_domain_withdrawn` — как
  снятие SSH host-key pin, #241): строка `promoted_by`/`promoted_at` уходит
  вместе с отзывом, и журнал — единственное, что остаётся;
...
- `GET /runs/{id}/org-profile` → владелец + связанные домены (пагинация через
  ...
  отсутствие данных;
- `POST /runs/{id}/related-domains/{domain}/promote` → добавить в цели всех
  последующих сканов тенанта (operator+, без автозапуска скана; `403`, если
  утверждённый scan scope домен не покрывает — по суффиксу или по адресу);
  `DELETE` по тому же пути — отозвать (run только называет тенанта);
  `promoted_domains` в ответе `org-profile` — **весь** promoted-список
  тенанта, не пересечение с кандидатами этого run'а;
- `GET /promoted-domains` (viewer, свой тенант) — что расширяет каждый скан:
  домен, run-источник, кто и когда; `DELETE /promoted-domains/{domain}`
  (operator+) — отозвать без run'а;
...
- ClickHouse: таблица `shapoclyack.shapoclyack_controls`
  (`api/services/ch_transform.py::controls_to_rows`) — по строке на контроль
  на run, `ORDER BY (tenant_id, control, timestamp, run_id)`, TTL 365 дней:
  тренд контроля во времени («почта была fail, стала weak»). Итоговый вердикт
  прогона денормализован в каждую строку, чтобы уровень run'а читался без
  второго запроса. Значения вне словаря `controls.py` схлопываются в
  `not_checked`/`unassessed` — инвариант «нет данных ≠ `ok`» держится и на
  стороне ClickHouse, а enum'ы таблицы не расширяются молча. Таблица
  создаётся не только init-скриптом первой загрузки, но и воркером при
  подключении (`ensure_controls_table`, `CREATE TABLE IF NOT EXISTS`) — иначе
  установка, обновлённая до #182, начала бы валить каждое ingest-сообщение;
  если DDL запрещён (read-only пользователь), строки контролей пропускаются, а
  ingest уязвимостей и портов продолжает работать.

**UI** (`web-next/src/`):

- `components/run/controls-matrix.tsx` + `controls-matrix.test.tsx` (тесты рядом
  с компонентом — уже принятая в репозитории практика);
- вкладка «Controls» на `/runs/view` — таблица с раскрытием в findings;
- вкладка «Org Profile» там же (`components/run/related-domains.tsx`) —
  владелец, блок «Promoted Scope» — весь promoted-список тенанта с «Withdraw
  from Scope» у каждого (виден и отзываем с любого run'а, не только с того,
  который домен предложил), связанные домены с evidence и кнопкой «Promote to
  Scope»; отказ scope (`403`) показывается баннером и снимается следующим
  успешным действием;
- страница `/org-profile` (`app/(dashboard)/org-profile/page.tsx`) — та же
  пара панелей на отдельном экране: по умолчанию новейший run, конкретный
  закрепляется через `?runId=`, дисклеймер атрибуции на самой странице, а не
  только в артефакте. Это целевой экран под новые данные поверх готовых
  компонентов, а не третий рерайт UI: запрет ROADMAP (*Not doing*) — про
  новый IA, и в эпике #182 страница названа явно;
- плитка со сводным уровнем на дашборде
  (`components/org-profile/posture-tile.tsx`) — вердикт новейшего run'а со
  ссылкой на `/org-profile`; run без стадии показывается как `not checked`,
  никогда как `ok`. Запрос делит ключ `run-controls` с матрицей, поэтому
  переход в run переиспользует кэш.
...
| **M4** ✅ | `related_domains.py` + promote-flow | поиск совместных доменов (SAN, CT, NS, MX), promote API и UI; продвинутые домены хранятся на тенанте (`0031`) и попадают в цели следующих сканов |
