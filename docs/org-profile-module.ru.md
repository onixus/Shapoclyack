...
- вкладка «Org Profile» там же (`components/run/related-domains.tsx`) —
  ...
  успешным действием. Отдельной страницы `/org-profile` нет — Track C закрыт,
  новых IA не заводим (ROADMAP, *Not doing*);
- плитка со сводным уровнем на дашборде — **не сделана**; сводка доступна на
  вкладке Controls конкретного run'а.
