...
A `register` that omits `capabilities` altogether leaves the stored list
alone — an agent that declares them only on the heartbeat must not lose them
by restarting — while one that sends a list replaces it.
