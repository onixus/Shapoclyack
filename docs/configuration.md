...
| `OCTO_METRICS_TENANT_TOP_N` | Per-tenant series on `/metrics` (`octo_tenant_open_findings`, `octo_tenant_sla_breached_findings`, `octo_tenant_scans_finished_24h`), default `0` = none. The N tenants with the most open findings plus scans in the last 24 h keep their id as the `tenant` label; all others are summed into `_other`. Capped at 50 (a larger value is clamped, with a warning), which bounds the label at 51 values. Under `prod` it requires `OCTO_METRICS_TOKEN` — the series name tenants — and the start is refused without it. See [observability.md](observability.md#per-tenant-series) |
