...
| `OCTO_AGENT_STALE_SECONDS` | How long after its last heartbeat an agent is reported `stale` (default `120`). Nothing is stored: the state is computed from `last_seen_at` at read time, so lowering it takes effect on the next request. It also decides the `agent_offline` event ([#349](https://github.com/onixus/Shapoclyack/issues/349)), and there it has a second effect: an agent is called *recovered*, and its offline alert closed, only after twice this long of unbroken heartbeats. Setting it below twice the agent's heartbeat interval (60s) makes every agent look stale between beats |
