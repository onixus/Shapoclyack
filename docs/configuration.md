...
| `OCTO_AGENT_KEEP_RUNS` | Keep each run's local copy after its result is in (or `--keep-runs`; default off). Read by the **sensor** (`agent/worker.py`), not the API. By default the sensor removes `runs/<run_id>` under `OCTO_OUTPUT_DIR` and under the scanner config's `runtime.state_dir` once the API accepted the upload or answered `409` (already ingesting this sensor's earlier copy, or declined); a failed upload keeps both until the same job, re-queued under the same run id, comes back to this sensor — then they are removed before the scan starts, with or without this setting, since the new attempt would otherwise be scanned into the old one's files. Removing them bounds the disk a sensor uses — before, every run it ever made stayed — but it also removes what delta discovery and the scanner's report diff read from the previous run: with `1` a delta scan skips the hosts found alive last time on this sensor, without it the whole scope is scanned ([k8s-hardening.md § What changes](k8s-hardening.md#what-changes)) |
