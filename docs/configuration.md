...
| **`OCTO_MASTER_KEY` unset while integration secrets are stored** — any `webhook_subscriptions` row carries a secret or a configured header (checked at startup, once the database is up) | Those rows hold somebody else's Jira / ServiceNow / DefectDojo token. Without a key they are written to Postgres as typed, so a dump or a replica hands them over; rows already encrypted cannot be read back at all. An installation with no integrations starts with a warning instead — this refuses "forgot to configure", not "does not use this" ([#310](https://github.com/onixus/Shapoclyack/issues/310)) |
...
| `OCTO_MASTER_KEY` | Key-encryption key for the integration secrets stored in Postgres — the webhook HMAC key and the header values that carry a tracker API token ([#310](https://github.com/onixus/Shapoclyack/issues/310)). 32 bytes as base64 or hex (`openssl rand -base64 32`); must be identical across API replicas. **Required in `prod`** once any such secret is stored. Unset, the values are written as typed and `dev` only warns. Replacing it without the step below makes every stored secret unreadable — see [operations.md § Secrets at rest](operations.md#secrets-at-rest) |
