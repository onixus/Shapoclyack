...
| `OCTO_HTTPS_PROXY` | Proxy for outgoing `https://` requests, in API **and** agent ([#359](https://github.com/onixus/Shapoclyack/issues/359)): webhook and ticket delivery, OIDC, advisory feeds, and every agent call. `[scheme://][user:pass@]host[:port]`; `http://` and `https://` proxies only — no SOCKS. Falls back to `HTTPS_PROXY`/`https_proxy` when unset; the prefixed name exists so a pod can override a cluster-wide injection meant for something else. HTTPS through it is a `CONNECT` tunnel, so the receiver's certificate is still verified against its own name |
| `OCTO_HTTP_PROXY` | The same for `http://` targets; falls back to `HTTP_PROXY`. The lowercase-only `http_proxy` is deliberately **not** read — in a CGI-shaped environment it is the caller's own `Proxy:` request header (httpoxy) |
