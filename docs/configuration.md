...
| `OCTO_MFA_STEPUP_MINUTES` | `15` | How recently the second factor must have been proved to mint or revoke a service token or a provisioning key, or to replace a tenant's scan scope. Applies **only** to accounts that have MFA enabled. Values below `1` are clamped to `1`: zero would demand a code per click, which teaches people to keep an authenticator open beside the console |
