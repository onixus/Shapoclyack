...
* График **Risk Trend** по снимкам `GET /api/vulnerabilities/risk-history` (окно задаётся `since`/`until`, по умолчанию последние 90 снимков) демонстрирует реальную динамику: снижается ли риск бизнеса или растет несмотря на закупку средств защиты.
...
2. **SLA Adherence (% соблюдения сроков):** доля уязвимостей, исправленных до дедлайна (`closed_within_sla_share`). Встроенные сроки платформы — Critical 15 дней, High 30, Medium 90, Low 180 (`DEFAULT_SLA_DAYS` в `api/services/vulnerabilities.py`); тенант переопределяет их политиками `sla_policies` по критичности актива и severity.
3. **Median Time to Remediate (MTTR):** медианное время устранения (`mttr_hours`) в разрезе severity (`mttr_hours_by_severity`). Разреза по бизнес-юнитам (`business_unit`) в `/adoption` нет — команду, которая тормозит исправления, CISO смотрит через фильтры `/vulnerabilities` по владельцу и бизнес-сервису актива.
4. **Блок чистоты (Noise & Exceptions):** количество принятых рисков (`accepted_open`). Принятие риска — в два лица: запрос с обязательным сроком и обоснованием, решение — роль `risk-approver` (`POST /api/vulnerabilities/{id}/exception/approve`), истечение снимается воркером SLA автоматически (миграция `0050`). Бессрочного принятия риска в платформе нет.
...
* **Compliance Posture Report (Отчет для аудиторов):** официальный отчет по состоянию контролей стандартов (PCI DSS 4.0, CIS Controls v8, ISO/IEC 27001, приказы ФСТЭК № 117 / 21 / 239, ГОСТ Р 57580.1). Содержит технические доказательства, даты проверок и прозрачный расчет Coverage Score.
...
CISO настраивает автоматическую генерацию и отправку отчетов по SMTP (`/api/reports/schedules`; доставка в S3/SFTP — [#355](https://github.com/onixus/Shapoclyack/issues/355)):
...
2. В разделе **Administration → Usage** (`/usage`) отслеживается потребление квот по тенантам: число активов (`max_assets`) и сканов за месяц (`max_scans_per_month`) с помесячной историей. Число сенсоров и агентов Lariska в квоту не входит — оно на странице «Флот удалённых агентов» (`/agents`) и в `/endpoints`.
