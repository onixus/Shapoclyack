...
2. В разделе **Administration → Usage** (`/usage`) отслеживается потребление квот по тенантам: число активов (`max_assets`) и сканов за месяц (`max_scans_per_month`) с помесячной историей. Число сенсоров и агентов Lariska в квоту не входит — оно на странице «Флот сенсоров» (`/agents`) и в `/endpoints`.
