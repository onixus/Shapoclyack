...
```mermaid
flowchart TD
    Start(["Старт процесса<br/>(Расписание / Триггер)"]) --> Scan["1. Сканирование периметра и хостов<br/>(сенсоры Shapoclyack и агент Lariska)"]
...
> **Сроки выше — регламентные, а не встроенные.** Встроенные значения платформы — Critical 15, High 30, Medium 90, Low 180 календарных дней (`DEFAULT_SLA_DAYS`, `api/services/vulnerabilities.py`); чтобы регламент работал, тенант задаёт свои сроки политиками `sla_policies` (по severity и критичности актива). Предупреждение о приближении дедлайна платформа выдаёт единообразно за 7 дней (`due_soon`, `DUE_SOON_DAYS`); для приказа ФСТЭК № 117 действуют отдельные сроки Руководства ФСТЭК (сигнал `overdue_fstec_window`). Колонки «Срок взятия в триаж» и «Предупреждение о просрочке» платформа не контролирует — это обязательства регламента.
...
sequenceDiagram
    ...
    Eng->>Shap: Запуск таргетированного скана (intent: vuln, цели и порты уязвимого сервиса)
...
* **T+0 ... T+2 часа:** идентификация сигнатуры угрозы (CVE ID, затронутый сервис и порты). Отдельного параметра «один Nuclei-шаблон по CVE» в `POST /api/jobs` нет — скан сужают целями (`ranges`/`domains`) и портами (`ports`).
* **T+2 ... T+6 часов:** запуск экспресс-сканирования всей внешней и внутренней инфраструктуры (`intent: vuln`); для хостов с агентом Lariska — обновление матчей ПО (`POST /api/endpoint/cve-matches/refresh`, фильтр `cve` в `GET /api/endpoint/cve-matches`). Оценка достижимости сервиса.
...
1. Заявка на принятие риска оформляется в карточке находки от имени владельца бизнес-системы — `POST /api/vulnerabilities/{id}/exception` (роль `admin` тенанта) с обязательными сроком (`until`) и обоснованием (`reason`).
2. Обязательные условия согласования:
   - Наличие компенсирующей меры (WAF, VPN, сегментация, мониторинг SIEM) — описывается в `reason`;
   - Указание конечного срока действия (регламент: не более 90 дней; платформа верхнюю границу не навязывает, только требует дату);
   - Решение принимает роль `risk-approver` (`/exception/approve` или `/exception/reject`) — второй человек, а не заявитель ([#348](https://github.com/onixus/Shapoclyack/issues/348)); для Critical/High регламент требует, чтобы им был CISO или уполномоченное им лицо.
3. По истечении срока исключение автоматически аннулируется воркером SLA (`exception_expired`, событие рабочего процесса — `api/services/workflow_events.py`), и просрочка снова начисляется.
