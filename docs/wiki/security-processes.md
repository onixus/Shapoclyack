...
* **Запрет одностороннего закрытия:** перевод задачи в статус «Решено» со стороны ИТ не приводит к автоматическому закрытию уязвимости в Shapoclyack.
* Задача переходит в статус «На проверке ИБ» (In Verification).
* Инженер ИБ инициирует автоматическую перепроверку через `POST /verify`.
* Если перепроверка не подтвердила исправление, задача возвращается разработчикам с обновленным логом сканирования.
