...
* **Никаких входящих портов на агентах:** Remote Agent инициирует только **исходящие** сессии — к FastAPI Control Plane по HTTPS и к NATS JetStream. Аутентификация в обоих случаях — агентский JWT, полученный в обмен на пер-тенантный provisioning key; **mTLS в сборке нет**, TLS для NATS появляется в [#309](https://github.com/onixus/Shapoclyack/issues/309) / [#359](https://github.com/onixus/Shapoclyack/issues/359). До этого брокер держат внутри доверенного сегмента, а между сегментами пускают только HTTPS-режим claim'а (`OCTO_NATS_URL` пустой).
...
* **Идемпотентность запусков:** использование заголовка `Idempotency-Key: commit-sha-build-id` исключает дублирование сканов при повторном запуске пайплайна в GitLab CI.
