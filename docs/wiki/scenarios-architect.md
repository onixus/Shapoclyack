...
* **Идемпотентность запусков:** использование заголовка `Idempotency-Key: commit-sha-build-id` исключает дублирование сканов при повторном запуске пайплайна в GitLab CI. Ключ принадлежит вызывающему — сервисному токену конвейера, — а не тенанту целиком, поэтому осмысленное имя вроде `nightly-triage` не пересекается с ключом соседнего конвейера того же заказчика.
