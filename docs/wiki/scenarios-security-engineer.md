...
2. Выберите группу сенсоров (`agent_group`; группы — `/api/agent-groups`, страница «Флот сенсоров», маршрут `/agents`), размещённых в целевом сетевом сегменте (DMZ, PCI DSS CDE, внутренний офис). Без группы задание берёт любой сенсор тенанта.
