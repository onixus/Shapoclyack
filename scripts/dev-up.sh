...
# k8s/octo-man/overlays/kind-dev (dev resources + local image tag + NodePort).
set -euo pipefail
...
echo "==> Applying k8s/octo-man/overlays/kind-dev"
kubectl apply -k k8s/octo-man/overlays/kind-dev

echo "==> Waiting for rollout"
kubectl -n "${NAMESPACE}" rollout status statefulset/octo-man-postgres --timeout=180s
kubectl -n "${NAMESPACE}" rollout status deployment/octo-man-api --timeout=180s
...
echo
echo "Logs:   kubectl -n ${NAMESPACE} logs deploy/octo-man-api -f"
