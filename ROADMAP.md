...
| **P3** | parallel track | Scale & observability | ~~Prometheus~~ (done, 3.4/3.5), OpenTelemetry, ~~SLOs~~ (done, 3.6), ~~server-side pagination~~ (done, 3.2/3.3), 1k/10k/50k-asset test fixtures, ClickHouse/API/UI profiling, coverage gate + frontend tests in CI |
...
Suggested order: 3.0 → 3.1 (cheap, CI-only, independent) → 3.4 (metrics — prerequisite for 3.6, useful for 3.8) → 3.2/3.3 (pagination, can run alongside 3.4) → 3.7 → 3.8 → 3.5/3.6 close it out. **Done so far: 3.0, 3.1, 3.2, 3.3, 3.4, 3.5, 3.6. Remaining: 3.7 (scale fixtures) → 3.8 (profiling), which also re-derive the provisional targets in [docs/slo.md](docs/slo.md).**
