...
**Status:** 10.1, 10.2 done; 10.3 webhooks done, ticketing bridges remaining.
...
| 10.3 | Workflow integrations | `api/services/integrations/{webhooks,delivery,webhook_worker}.py` (new), `api/routes/webhooks.py` (new), migration `0011` | **Webhooks done.** Per-tenant subscriptions carry the routing policy (event kinds + a `min_severity` floor that applies only to the kinds that have a severity, so a "critical only" rule cannot silently swallow a decommission). A JetStream durable consumer on `events.asset.>` queues matching events and acks; a separate dispatcher delivers them, so a slow receiver never stalls consumption of the stream. `webhook_deliveries` is queue, DLQ and audit trail in one table — the same rows under different predicates. Retries exponential and capped; 5xx/408/429/timeouts retry, every other 4xx is dead-lettered on the first attempt. Dispatch runs in every replica without leader election (claims via `FOR UPDATE SKIP LOCKED` + a visibility timeout), and DLQ replay works with the broker down. Payloads are HMAC-signed with the timestamp inside the MAC; the secret is returned once and write-only after. Webhook targets resolving to non-global addresses are refused at write time and again before every POST (SSRF), redirects not followed. Writes need tenant `admin`. **Remaining:** Jira/ServiceNow ticket creation on new critical exposure, and extending the existing DefectDojo export — both are further transports over this same queue | **Partial** |
...
| **P2** | 2–3 sprints | Asset event workflows | In progress: ~~`events.asset.*` bus~~ (done, [10.2](#phase-10--change-detection--alerting-at-asset-level)); ~~routing policies, webhooks, retries, DLQ, audit trail~~ (done, [10.3](#phase-10--change-detection--alerting-at-asset-level)); remaining — Jira/ServiceNow ticket creation and the DefectDojo export, as further transports over the same delivery queue |
...
Suggested order: 4.1 (done) → 4.2 (done) → 4.3 (done on branch) → 4.4 (done; the only item that adds a new data-protection surface).
...
| IP↔FQDN↔certificate correlation | [P4.2](#p4-breakdown--differentiating-features) | **Done on branch** — feeds Track C's asset model ([#146](https://github.com/onixus/Shapoclyack/issues/146)); see [docs/asset-identity.md](docs/asset-identity.md) |
| Ownership graph | [P4.3](#p4-breakdown--differentiating-features) | **Done on branch** — groups the 11.2 graph by operator-set unit/owner; unowned names by registrable domain |
...
| Issue | Layer | Scope |
|-------|-------|-------|
| ~~[#144](https://github.com/onixus/Shapoclyack/issues/144)~~ | Backend | **Done** — scoring model `nist-1`: NIST SP 800-30 likelihood × impact through Table I-2, exploit maturity (`attacked`/`weaponized`/`proof_of_concept`/`unproven`/`theoretical`/`unknown`) with a named evidence trail, and asset criticality moved onto the impact axis where it can change the verdict. Methodology and its stated limits: [docs/risk-scoring.md](docs/risk-scoring.md). Remaining scope split out as ~~[#171](https://github.com/onixus/Shapoclyack/issues/171)~~ (**done on branch** — host reachability as a likelihood input; public IP is not `external`), ~~[#172](https://github.com/onixus/Shapoclyack/issues/172)~~ (**done on branch** — CVE publication age, raise-only; overlay staleness visible), ~~[#173](https://github.com/onixus/Shapoclyack/issues/173)~~ (**done on branch** — compensating controls as a named −6 when fingerprint saw a CDN/WAF on the same host:port; same-asset path +8 when a local finding shares a P4.2 asset with a network foothold; not a domain-takeover model). Historical score snapshots for trend charts are **not** done |
...
| [#146](https://github.com/onixus/Shapoclyack/issues/146) | Backend | **Done on branch** — asset business context (`business_service`, `environment`, `data_classification`, `exposure_level`, `context_source`) plus a same-transaction audit trail and a per-asset risk rollup. CMDB/AD use the same PATCH. Exposure is an operator decision, not a scan fact ([#171](https://github.com/onixus/Shapoclyack/issues/171)); identity merge is [P4.2](#p4-breakdown--differentiating-features) (done on branch). Docs: [docs/asset-context.md](docs/asset-context.md) |
| [#135](https://github.com/onixus/Shapoclyack/issues/135) | UI | **Done on branch** — Risk Overview (`/`) on tracked findings: estate NIST verdict, SLA, unassigned work, unowned assets. Internet exposure as a measurement stays [#171](https://github.com/onixus/Shapoclyack/issues/171); historical risk snapshots stay [#144](https://github.com/onixus/Shapoclyack/issues/144) |
...
| [#137](https://github.com/onixus/Shapoclyack/issues/137) | UI | **Done on branch** — Vulnerability Center (`/vulnerabilities`, `/vulnerabilities/view`) on top of #145: lifecycle stepper, owner, SLA, risk acceptance, audit trail. CWE is copied from the last observation (NVD overlay, else nuclei) |
...
| [#139](https://github.com/onixus/Shapoclyack/issues/139) | UI | **Done on branch** — MSSP tenant posture comparison, declared-exposure inventory, KEV threat intel. Attack paths stay [#173](https://github.com/onixus/Shapoclyack/issues/173) (chaining still open; compensating controls landed on branch); internet as a scan fact stays [#171](https://github.com/onixus/Shapoclyack/issues/171) |
