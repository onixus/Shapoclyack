...
| **P1** | 2–4 sprints | Durable control plane | ~~Move jobs and agents into PostgreSQL~~ (done, 1.1/1.2); ~~formal state machine~~ (done, 1.3); ~~leases~~ (done, 1.4); idempotency keys; extract the scheduler into its own worker or add leader election — see [breakdown](#p1-breakdown--durable-control-plane) |
...
Suggested order: ~~1.3 → 1.4~~ (both done) → 1.5 → 1.6 (independent of the rest; can run in parallel).
