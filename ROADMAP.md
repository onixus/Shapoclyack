...
| Issue | Layer | Scope |
|-------|-------|-------|
| ~~[#144](https://github.com/onixus/Shapoclyack/issues/144)~~ | Backend | **Done** — scoring model `nist-1`: NIST SP 800-30 likelihood × impact through Table I-2, exploit maturity (`attacked`/`weaponized`/`proof_of_concept`/`unproven`/`theoretical`/`unknown`) with a named evidence trail, and asset criticality moved onto the impact axis where it can change the verdict. Methodology and its stated limits: [docs/risk-scoring.md](docs/risk-scoring.md). Remaining scope split out as ~~[#171](https://github.com/onixus/Shapoclyack/issues/171)~~ (**done on branch** — host reachability as a likelihood input; public IP is not `external`), ~~[#172](https://github.com/onixus/Shapoclyack/issues/172)~~ (**done on branch** — CVE publication age, raise-only; overlay staleness visible), [#173](https://github.com/onixus/Shapoclyack/issues/173) (chaining, compensating controls). Historical score snapshots for trend charts are **not** done |
