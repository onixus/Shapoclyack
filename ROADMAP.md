...
| **P4** | 3–5 sprints | Differentiating features | In progress: ~~TLS hostname/SAN-CN mismatch check~~ (done, 4.1); ~~IP↔FQDN↔certificate correlation~~ (done, 4.2); ~~ownership graph~~ (done on branch, 4.3); web screenshots with retention/redaction (closes [9.3](#phase-9--exposure-fingerprinting)); ~~risk-priority explanation~~ (done — `risk_explanation` from scoring model `mvp-2`, see [docs/pulse-backend.md](docs/pulse-backend.md)) — see [breakdown](#p4-breakdown--differentiating-features) |
...
Suggested order: 4.1 (done) → 4.2 (done) → 4.3 (done on branch) → 4.4 last (heaviest, and the only item that adds a new data-protection surface).
