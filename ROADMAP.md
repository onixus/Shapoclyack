...
| **P1** | 2–4 sprints | Durable control plane | ~~Move jobs and agents into PostgreSQL~~ (done, 1.1/1.2); ~~formal state machine~~ (done, 1.3); ~~leases~~ (done, 1.4); ~~idempotency keys~~ (done, 1.5); extract the scheduler into its own worker or add leader election — see [breakdown](#p1-breakdown--durable-control-plane) |
...
P0 is complete; P1 is in progress and remains a prerequisite for safely running the platform at MSSP scale; P2 completes the EASM alerting loop already scaffolded in Phase 10; P3 is a parallel hardening track, not a blocking dependency; P4 is scope already flagged as deferred/out-of-scope in Phases 9–10. P4.1 landed ahead of that ordering because it needed nothing from P1/P2 — it reads certificates the scanner had already collected — but the rest of P4 keeps the original dependency: 4.2 feeds asset identity and 4.3 the alerting graph, and neither is worth building on a control plane that cannot yet run two replicas.

### P1 breakdown — Durable control plane

**Why this blocks MSSP scale:** every manifest in `k8s/` runs `replicas: 1`, and until 1.1/1.2 that was not a sizing choice but a correctness requirement — the job queue and the agent registry were per-process dicts, so a second replica meant a second control plane. 1.1/1.2 removed that constraint for *state*, and 1.4 makes a replica's death recoverable rather than permanent; dispatcher leader election (1.6) is the last piece before a second replica is actually safe to run.
...
| 1.6 | Scheduler leader election | `api/services/schedule_dispatcher.py` | The dispatcher runs in-process in **every** replica with no coordination, so N replicas wake for the same due schedule. Since 1.5 they no longer produce N *scans* (each dispatch is keyed on the schedule's due time), but they still all poll and still race on the schedule's own bookkeeping. Postgres advisory lock (or extract the dispatcher into its own single-replica worker). Until this lands, run one API replica or set `OCTO_SCHEDULER_DISPATCH_ENABLED=false` on all but one | Planned |

Suggested order: ~~1.3 → 1.4 → 1.5~~ (all done) → 1.6, the last piece (independent of the rest).
